  ...
  let result: ShortestPathResponse[] = (
    await Promise.all(
      cinemas.map(async (c) => {
        ...
        const shortest_path = await get_path_by_nodes(shortest_path_neo.path.join(", "));
        if (!shortest_path) {
          return null;
        }
        return new ShortestPathResponse(c.id, shortest_path, shortest_path_neo.total_cost);
      })
    )
  )
    .filter((sp) => sp != null)
